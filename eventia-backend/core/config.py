...
# @Date:   2025-04-18 19:52:51
# @Last Modified by:   Roni Laukkarinen
# @Last Modified time: 2025-04-18 19:54:41
"""
Application configuration settings module.

This module handles all environment-based configuration settings for the Eventia API,
including database connections, API settings, and feature flags.
"""
import logging
import os
import secrets
...
class Settings(BaseSettings):
    """Application settings for the Eventia API.
    
    This class manages all configuration settings, sourced from environment
    ...
    def assemble_cors_origins(cls, v: Union[str, List[str]]) -> Union[List[str], str]:
        """Parse CORS settings from string or list format."""
    ...
    def assemble_mongo_uri(cls, v: Optional[str], values: Dict[str, Any]) -> str:
        """Build MongoDB connection URI from components if not explicitly provided."""
        if v:
            return v
    ...
    def validate_sentry_dsn(cls, v: Optional[str]) -> Optional[str]:
        """Validate and clean Sentry DSN configuration."""
        if v is None or v == "":
            return None
        return v
...
def get_settings() -> Settings:
    ...
    Uses LRU cache to prevent reloading settings on every request.
    """
