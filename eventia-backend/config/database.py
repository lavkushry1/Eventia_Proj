...
def create_indexes():
    ...
    try:
        ...
        # Stadiums collection
        sync_db.stadiums.create_index([("stadium_id", pymongo.ASCENDING)], unique=True)
        
        # Stadium views collection
        sync_db.stadium_views.create_index([
            ("stadium_id", pymongo.ASCENDING),
            ("view_id", pymongo.ASCENDING)
        ], unique=True)
...
async def connect_to_mongo():
    ...
    try:
        ...
        print(f"Failed to connect to MongoDB at {settings.MONGO_URI}")
        return False
    except Exception as e:
...
def init_default_settings():
    ...
    try:
        ...
        if not sync_db.settings.find_one({"type": "payment_settings"}):
            ...
            print("Initialized default payment settings")
        ...
        if not sync_db.settings.find_one({"type": "about_page"}):
            ...
            print("Initialized default about page content")
    
    except Exception as e:
        print(f"Error initializing default settings: {str(e)}") 